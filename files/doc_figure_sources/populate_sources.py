...
def create_plots(one: OneDoc):
    ...
    fig4 = px.scatter(
        df4, x="Parameter X", y="Response", color="Condition", title="Primary Experimental Results and Trends"
    )
    ...
    fig7.update_layout(
        title="Comparison between theoretical predictions and experimental results",
    ...
    np.random.seed(456)
    df8 = pd.DataFrame(
        {"Component": ["Frontend", "Backend", "Database", "Cache", "Queue"], "Load": np.random.randint(20, 95, 5)}
    )
    ...
    for config in configs:
        for metric in metrics:
            data.append({"Configuration": config, "Metric": metric, "Score": np.random.uniform(60, 95)})
    ...
    fig15 = px.box(
        df15, x="Condition", y="Value", color="Group", title="Statistical Distributions by Condition and Group"
    )
