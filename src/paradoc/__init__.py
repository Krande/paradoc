...
class OneDoc:
    ...
    def compile(self, output_name, onedrive_dir=None, auto_open=False, metadata_file=None):
        """

        :param output_name: Name of output document
        :param onedrive_dir: Optionally sync with your onedrive
        :param auto_open:
        :param metadata_file:
        """
        ...
        # new_docx = Document(dest_file)
        # new_docx.save(dest_file.parent / f'{dest_file.name}_fixed.docx')

        # Save and upload to onedrive
        if onedrive_dir is not None:
            try:
                from ais import OneConnect
            except ModuleNotFoundError:
                raise ModuleNotFoundError(
                    "OneDrive integration requires installation of ais. "
                    "See https://azapp-ada-book-001.azurewebsites.net/doc/main/pypackages/ais.html"
                )
            one = OneConnect()
            one.upload_file(self.dist_dir / output_name, onedrive_dir)
