...
class Composer(object):
    ...
    def add_relationship(self, src_part, dst_part, relationship):
        ...
        if relationship.is_external:
            new_rid = dst_part.rels.get_or_add_ext_rel(relationship.reltype, relationship.target_ref)
    ...
    def footnote_part(self):
        ...
        try:
            ...
            xml_path = os.path.join(os.path.dirname(__file__), "templates", "footnotes.xml")
            with open(xml_path, "rb") as f:
                xml_bytes = f.read()
            footnote_part = Part(partname, content_type, xml_bytes, self.doc.part.package)
    ...
    def add_styles(self, doc, element):
        ...
        used_style_ids = list(
            OrderedDict.fromkeys([e.val for e in xpath(element, ".//w:tblStyle|.//w:pStyle|.//w:rStyle")])
        )
    ...
    def add_numberings(self, doc, element):
        ...
        for num_id in num_ids:
            ...
            if anum_id.val not in self.anum_id_mapping:
                ...
                res = src_numbering_part.element.xpath('.//w:abstractNum[@w:abstractNumId="%s"]' % anum_id.val)
                if not res:
                    continue
                ...
                if nsid is not None:
                    nsid.set("{%s}val" % NS["w"], "{0:08X}".format(int(10**8 * random.random())))
    ...
    def _next_numbering_ids(self):
        ...
        current_anum_ids = [int(n) for n in xpath(numbering_part.element, ".//w:abstractNum/@w:abstractNumId")]
        if current_anum_ids:
    ...
    def numbering_part(self):
        ...
        try:
            ...
            xml_path = os.path.join(os.path.dirname(__file__), "templates", "numbering.xml")
            with open(xml_path, "rb") as f:
                xml_bytes = f.read()
            element = parse_xml(xml_bytes)
            numbering_part = NumberingPart(partname, content_type, element, self.doc.part.package)
    ...
    def restart_first_numbering(self, doc, element):
        ...
        anum_element = xpath(numbering_part.element, './/w:abstractNum[@w:abstractNumId="%s"]' % anum_id)
    ...
    def header_part(self, content=None):
        ...
        header_rels = [rel for rel in self.doc.part.rels.values() if rel.reltype == RT.HEADER]
        next_id = len(header_rels) + 1
        # Create a new header part
        partname = PackURI("/word/header%s.xml" % next_id)
        content_type = CT.WML_HEADER
        if not content:
            xml_path = os.path.join(os.path.dirname(__file__), "templates", "header.xml")
            with open(xml_path, "rb") as f:
                content = f.read()
    ...
    def footer_part(self, content=None):
        ...
        footer_rels = [rel for rel in self.doc.part.rels.values() if rel.reltype == RT.FOOTER]
        next_id = len(footer_rels) + 1
        # Create a new header part
        partname = PackURI("/word/footer%s.xml" % next_id)
        content_type = CT.WML_FOOTER
        if not content:
            xml_path = os.path.join(os.path.dirname(__file__), "templates", "footer.xml")
            with open(xml_path, "rb") as f:
                content = f.read()
