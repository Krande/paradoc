...
def test_word_com_api_wrapper(tmp_path):
    ...
    with WordApplication(visible=False) as word_app:
        ...
        fig_bookmark = doc.add_figure_with_caption(caption_text="Example Figure", create_bookmark=True)

        doc.add_paragraph()

        # Add a table with caption
        table_bookmark = doc.add_table_with_caption(caption_text="Example Table", rows=3, cols=3, create_bookmark=True)

        doc.add_paragraph()

        # Add cross-references
        doc.add_cross_reference(bookmark_name="figure_0", reference_type="figure", prefix_text="See ")
        doc.add_paragraph(" for the figure.")

        doc.add_paragraph()
        doc.add_cross_reference(bookmark_name="table_0", reference_type="table", prefix_text="Refer to ")
...
def test_table_with_data(tmp_path):
    ...
    with WordApplication(visible=False) as word_app:
        ...
        table_bookmark = doc.add_table_with_caption(
            caption_text="Test Data Table", rows=3, cols=3, data=table_data, create_bookmark=True
        )
        ...
        doc.add_table_with_caption(caption_text="Numeric Data Table", rows=4, cols=3, data=numeric_data)
    ...
    with WordApplication(visible=False) as word_app:
        ...
        with pytest.raises(ValueError, match="Data has 2 rows but table has 3 rows"):
            doc.add_table_with_caption(
                caption_text="Invalid Table", rows=3, cols=2, data=[["A", "B"], ["C", "D"]]  # Only 2 rows instead of 3
            )
...
def test_figure_layouts(tmp_path):
    ...
    with WordApplication(visible=False) as word_app:
        ...
        doc.add_figure_with_caption(caption_text="Inline Figure", layout=FigureLayout.INLINE)
        doc.add_paragraph()

        # Test square wrapping
        doc.add_paragraph("Figure with square wrapping:")
        doc.add_figure_with_caption(caption_text="Square Wrapped Figure", layout=FigureLayout.SQUARE)
        doc.add_paragraph()

        # Test tight wrapping
        doc.add_paragraph("Figure with tight wrapping:")
        doc.add_figure_with_caption(caption_text="Tight Wrapped Figure", layout=FigureLayout.TIGHT)
        doc.add_paragraph()

        # Test top/bottom wrapping
        doc.add_paragraph("Figure with top/bottom wrapping:")
        doc.add_figure_with_caption(caption_text="Top/Bottom Wrapped Figure", layout=FigureLayout.TOP_BOTTOM)
        doc.add_paragraph()

        # Test behind text
        doc.add_paragraph("Figure behind text:")
        doc.add_figure_with_caption(caption_text="Behind Text Figure", layout=FigureLayout.BEHIND_TEXT)
        doc.add_paragraph()

        # Test in front of text
        doc.add_paragraph("Figure in front of text:")
        doc.add_figure_with_caption(caption_text="In Front of Text Figure", layout=FigureLayout.IN_FRONT_OF_TEXT)
        doc.add_paragraph()

        # Test with string parameter
        doc.add_paragraph("Figure using string parameter:")
        doc.add_figure_with_caption(
            caption_text="String Parameter Figure", layout="square"  # Pass as string instead of enum
        )
...
def test_chapter_numbering(tmp_path):
    ...
    with WordApplication(visible=False) as word_app:
        ...
        doc.add_figure_with_caption(
            caption_text="Second figure in chapter 1", use_chapter_numbers=True, create_bookmark=False
        )
        doc.add_paragraph()

        # Add table with chapter numbering in Chapter 1
        doc.add_table_with_caption(
            caption_text="First table in chapter 1", rows=2, cols=2, use_chapter_numbers=True, create_bookmark=False
        )
        doc.add_paragraph()
        ...
        doc.add_figure_with_caption(
            caption_text="First figure in chapter 2", use_chapter_numbers=True, create_bookmark=False
        )
        doc.add_paragraph()

        doc.add_figure_with_caption(
            caption_text="Second figure in chapter 2", use_chapter_numbers=True, create_bookmark=False
        )
        doc.add_paragraph()

        # Add table with chapter numbering in Chapter 2
        doc.add_table_with_caption(
            caption_text="First table in chapter 2", rows=2, cols=2, use_chapter_numbers=True, create_bookmark=False
        )
        doc.add_paragraph()
        ...
        doc.add_figure_with_caption(
            caption_text="Figure with chapter numbering", use_chapter_numbers=True, create_bookmark=False
        )
        doc.add_paragraph()
    ...
    assert (
        "Table 2:" in table_captions[2]
    ), f"Expected 'Table 2:' in third caption (simple numbering), got: {table_captions[2]}"
    ...
    print("  Chapter 3: Figure 3-1, Table 2 (simple)")
...
def test_cross_references_with_chapter_numbering(tmp_path):
    ...
    with WordApplication(visible=False) as word_app:
        ...
        fig1_bookmark = doc.add_figure_with_caption(
            caption_text="First figure in chapter 1", use_chapter_numbers=True, create_bookmark=True
        )
        doc.add_paragraph()

        fig2_bookmark = doc.add_figure_with_caption(
            caption_text="Second figure in chapter 1", use_chapter_numbers=True, create_bookmark=True
        )
        doc.add_paragraph()

        # Add table with chapter numbering and bookmark
        tbl1_bookmark = doc.add_table_with_caption(
            caption_text="First table in chapter 1", rows=2, cols=2, use_chapter_numbers=True, create_bookmark=True
        )
        doc.add_paragraph()
        ...
        doc.add_cross_reference(bookmark_name=0, reference_type="figure", include_hyperlink=True)  # First figure
        doc.add_text(", the data clearly demonstrates the trend.")
        doc.add_paragraph()

        doc.add_text("Additionally, ")
        doc.add_cross_reference(
            bookmark_name=1, reference_type="figure", include_hyperlink=True, prefix_text=""  # Second figure
        )
        doc.add_text(" provides further evidence.")
        doc.add_paragraph()

        doc.add_text("The results are summarized in ")
        doc.add_cross_reference(bookmark_name=0, reference_type="table", include_hyperlink=True)  # First table
        doc.add_text(".")
        doc.add_paragraph()

        # Add figures in Chapter 2
        fig3_bookmark = doc.add_figure_with_caption(
            caption_text="First figure in chapter 2", use_chapter_numbers=True, create_bookmark=True
        )
        doc.add_paragraph()

        tbl2_bookmark = doc.add_table_with_caption(
            caption_text="First table in chapter 2", rows=2, cols=2, use_chapter_numbers=True, create_bookmark=True
        )
        doc.add_paragraph()
        ...
        doc.add_cross_reference(bookmark_name=0, reference_type="figure", include_hyperlink=True)  # Figure 1-1
        doc.add_text(" and ")
        doc.add_cross_reference(bookmark_name=2, reference_type="figure", include_hyperlink=True)  # Figure 2-1
        doc.add_text(" reveals important differences.")
        doc.add_paragraph()

        # Reference tables
        doc.add_text("See ")
        doc.add_cross_reference(bookmark_name=0, reference_type="table", include_hyperlink=True)  # Table 1-1
        doc.add_text(" and ")
        doc.add_cross_reference(bookmark_name=1, reference_type="table", include_hyperlink=True)  # Table 2-1
    ...
    assert (
        "Figure 1-1" in crossref_text or "Figure 1 1" in crossref_text
    ), "Expected cross-reference to 'Figure 1-1' in text"
    assert (
        "Figure 1-2" in crossref_text or "Figure 1 2" in crossref_text
    ), "Expected cross-reference to 'Figure 1-2' in text"
    assert (
        "Figure 2-1" in crossref_text or "Figure 2 1" in crossref_text
    ), "Expected cross-reference to 'Figure 2-1' in text"

    # Should contain references to Table 1-1, Table 2-1
    assert (
        "Table 1-1" in crossref_text or "Table 1 1" in crossref_text
    ), "Expected cross-reference to 'Table 1-1' in text"
    assert (
        "Table 2-1" in crossref_text or "Table 2 1" in crossref_text
    ), "Expected cross-reference to 'Table 2-1' in text"
    ...
    for forbidden in forbidden_caption_texts:
        assert (
            forbidden not in crossref_text
    ...
    for pat in colon_patterns:
        assert (
            pat not in crossref_text
        ), f"Cross-references should not include caption-style colon text, but found '{pat}'"

    print("\n[PASS] All cross-reference assertions passed!")
    print("  Captions: Figure 1-1, Figure 1-2, Figure 2-1, Table 1-1, Table 2-1")
    print("  Cross-references verified in document text (without caption text)")
...
def test_caption_reference_wrapper(tmp_path):
    ...
    with WordApplication(visible=False) as word_app:
        ...
        fig1 = doc.add_figure_with_caption(
            caption_text="System architecture diagram", use_chapter_numbers=True, create_bookmark=True
        )
        doc.add_paragraph()

        fig2 = doc.add_figure_with_caption(
            caption_text="Data flow diagram", use_chapter_numbers=True, create_bookmark=True
        )
        doc.add_paragraph()
        ...
        fig3 = doc.add_figure_with_caption(
            caption_text="Implementation flowchart", use_chapter_numbers=True, create_bookmark=True
        )
        doc.add_paragraph()
    ...
    print("  Successfully used CaptionReference objects for cross-referencing")
    print("  No need to specify reference_type manually!")
