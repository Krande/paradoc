...
"""
    ...
    print("[DEBUG] This indicates the cross-reference conversion is not working properly")
    ...
    for i, para in enumerate(doc_after.paragraphs):
        ...
        if fig_matches:
            paragraphs_with_refs.append(
                {"index": i, "text": text, "type": "figure", "refs": [f"Figure {ch}-{num}" for ch, num in fig_matches]}
            )
        ...
        if tbl_matches:
            paragraphs_with_refs.append(
                {"index": i, "text": text, "type": "table", "refs": [f"Table {ch}-{num}" for ch, num in tbl_matches]}
            )
    ...
    para_fig_trends_1 = next(
        (p for p in paragraphs_with_refs if "historical trends demonstrate a clear pattern" in p["text"]), None
    )
    ...
    para_fig_analysis = next(
        (p for p in paragraphs_with_refs if "analysis reveals important insights" in p["text"]), None
    )
    ...
    para_fig_projection = next(
        (p for p in paragraphs_with_refs if "projection" in p["text"].lower() and "future trends" in p["text"]), None
    )
    ...
    assert (
        len(para_fig_projection["refs"]) == 4
    ...
    para_tbl_metrics_1 = next(
        (p for p in paragraphs_with_refs if "supports this observation" in p["text"] and p["type"] == "table"), None
    )
    ...
    para_tbl_comparison = next(
        (
            p
            for p in paragraphs_with_refs
            if "performance comparison" in p["text"].lower() and "significant improvements" in p["text"]
        ),
        None,
    )
    assert (
        para_tbl_comparison is not None
    ), "Could not find paragraph with 'performance comparison' and 'significant improvements'"
    print(f"\n✓ Found: '{para_tbl_comparison['text'][:60]}...'")
    print(f"  References: {para_tbl_comparison['refs']}")
    assert (
        "Table 2-1" in para_tbl_comparison["refs"]
    ), f"Expected 'Table 2-1' (comparison) in this paragraph, got {para_tbl_comparison['refs']}"
    assert (
        "Table 1-1" in para_tbl_comparison["refs"]
    ), f"Expected 'Table 1-1' (metrics) in this paragraph, got {para_tbl_comparison['refs']}"
    assert (
        len(para_tbl_comparison["refs"]) == 2
    ...
    para_tbl_summary = next(
        (
            p
            for p in paragraphs_with_refs
            if "baseline metrics" in p["text"].lower() and "performance comparison" in p["text"].lower()
        ),
        None,
    )
    ...
    assert (
        "Table 1-1" in para_tbl_summary["refs"]
    ), f"Expected 'Table 1-1' (metrics) in summary paragraph, got {para_tbl_summary['refs']}"
    assert (
        "Table 2-1" in para_tbl_summary["refs"]
    ), f"Expected 'Table 2-1' (comparison) in summary paragraph, got {para_tbl_summary['refs']}"
    assert (
        "Table 3-1" in para_tbl_summary["refs"]
    ), f"Expected 'Table 3-1' (summary) in summary paragraph, got {para_tbl_summary['refs']}"
    assert (
        len(para_tbl_summary["refs"]) == 3
    ), f"Expected exactly 3 table references, got {len(para_tbl_summary['refs'])}"

    print("\n" + "=" * 80)
    print("TEST PASSED - ALL CROSS-REFERENCES VERIFIED")
    print("=" * 80)
    print("✓ Figure reference to 'trends' correctly points to Figure 1-1")
    print("✓ Figure reference to 'analysis' correctly points to Figure 2-1")
    print("✓ Figure reference to 'projection' correctly points to Figure 2-2")
    print("✓ Table reference to 'metrics' correctly points to Table 1-1")
    print("✓ Table reference to 'comparison' correctly points to Table 2-1")
    print("✓ Table reference to 'summary' correctly points to Table 3-1")
    print("✓ All multi-reference paragraphs have correct target references")
    print("✓ All REF fields properly structured with \\h switch")
    print("✓ All bookmarks use Word-style _Ref naming")
    ...
    for para in doc.paragraphs:
        ...
        for begin_idx in field_begins:
            ...
            if instr_text and " REF " in instr_text:
                ...
                ref_fields.append({"instr": instr_text, "result_text": result_text, "paragraph_text": para.text})
