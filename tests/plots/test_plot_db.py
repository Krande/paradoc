...
class TestPlotData:
    ...
    def test_plot_data_creation(self):
        ...
        plot = PlotData(key="test_plot", plot_type="line", data={"x": [1, 2, 3], "y": [4, 5, 6]}, caption="Test Plot")
    ...
    def test_plot_data_with_dimensions(self):
        ...
        plot = PlotData(key="sized_plot", plot_type="bar", data={}, caption="Sized Plot", width=800, height=600)
    ...
    def test_plot_data_key_validation(self):
        ...
        with pytest.raises(ValueError, match="should not contain __ markers"):
            PlotData(key="__invalid__", plot_type="line", data={}, caption="Invalid")
...
class TestPlotUtilities:
    ...
    def test_dataframe_to_plot_data(self):
        ...
        plot = dataframe_to_plot_data(
            key="df_plot", df=df, plot_type="line", caption="DataFrame Plot", width=700, height=500
        )
...
class TestDbManagerPlots:
    ...
    def test_list_plots(self):
        ...
        # Add multiple plots
        for i in range(3):
            plot = PlotData(key=f"plot_{i}", plot_type="bar", data={}, caption=f"Plot {i}")
            self.db_manager.add_plot(plot)
    ...
    def test_update_plot(self):
        ...
        # Add plot
        plot = PlotData(key="update_test", plot_type="line", data={"x": [1, 2]}, caption="Original Caption")
        self.db_manager.add_plot(plot)

        # Update plot
        updated_plot = PlotData(
            key="update_test", plot_type="bar", data={"x": [1, 2, 3, 4]}, caption="Updated Caption", width=1000
        )
    ...
    def test_delete_plot(self):
        ...
        # Add plot
        plot = PlotData(key="delete_test", plot_type="scatter", data={}, caption="To Be Deleted")
        self.db_manager.add_plot(plot)
...
class TestPlotRenderer:
    ...
    def test_create_line_plot(self):
        ...
        plot_data = dataframe_to_plot_data(
            key="line_test", df=df, plot_type="line", caption="Line Plot Test", width=600, height=400
        )
    ...
    def test_create_bar_plot(self):
        ...
        plot_data = dataframe_to_plot_data(key="bar_test", df=df, plot_type="bar", caption="Bar Plot Test")
    ...
    def test_plot_with_annotation_override(self):
        ...
        # plot_data = PlotData(
        #     key="override_test", plot_type="scatter", data={}, caption="Override Test", width=800, height=600
        # )
...
class TestPlotIntegration:
    ...
    def test_full_plot_workflow(self):
        ...
        # Create plot data
        df = pd.DataFrame({"x": [1, 2, 3, 4, 5], "y": [2, 4, 3, 5, 6]})

        plot_data = dataframe_to_plot_data(
            key="workflow_test", df=df, plot_type="line", caption="Workflow Test Plot", width=700, height=500
        )
    ...
    def test_multiple_plots_same_type(self):
        ...
        for i in range(5):
            df = pd.DataFrame({"x": list(range(i + 1)), "y": [j**2 for j in range(i + 1)]})

            plot_data = dataframe_to_plot_data(key=f"multi_plot_{i}", df=df, plot_type="scatter", caption=f"Plot {i}")
    ...
    def test_plot_metadata(self):
        ...
        plot_data = PlotData(
            ...
            metadata={"source": "test_suite", "created_by": "unit_test", "version": "1.0"},
        )
