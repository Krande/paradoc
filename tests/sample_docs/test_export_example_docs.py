...
def test_doc1_export_to_html(files_dir, test_dir, export_format):
    source = files_dir / "doc1"
    dest = test_dir / f"{source.name}/{source.name}.{export_format}"
...
@pytest.mark.parametrize("export_format", ["html", "docx", "pdf"])
def test_doc2_export_to_html(files_dir, test_dir, export_format):
